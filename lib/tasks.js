...
async function scanReactions (client) {
  ...
  log.info('Submission channel match for ' + config.submissionChannel + ' is ' + submissionChannel)
  const approvalChannel = client.channels.find(c => c.name === config.approvedChannel)
  log.info('Approval channel match for ' + config.approvedChannel + ' is ' + approvalChannel)
