...
const { log, createEmbed, sendDM, findMessagesForChannel, findBuildMessage, buildMessageLink } = require('./common')
...
module.exports = async (message, command, args) => {
  ...
  switch (command) {
    ...
    case 'post': {
      ...
      const messages = await findMessagesForChannel(message.client, config.submissionChannel)
    ...
    case 'build': {
      ...
      const messages = await findMessagesForChannel(message.client, config.approvedChannel)
      const buildMessage = findBuildMessage(value, messages)
    ...
    case 'build-full': {
      ...
      if (buildMessage) {
        return message.channel.send({
          embed: new Discord.MessageEmbed(buildMessage.embeds[0])
        })
      }
